...
<div class="row">
  <div class="col-sm-12 col-md-8 col-md-offset-2">
    ...
    @foreach( $servers as $server )
      <h2 class="page-header">{{ $server->hostname }} <small>{{ $server->env }}</small></h2>
      <ul class="list-group">
        @if(count($server->applications) > 0)
          @foreach($server->applications as $application)
            <li class="list-group-item">
              <span>
                <a href="{{ url('admin/applications', $application->id)}}">{{ $application->name }} <span class="pull-right"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span></a>
              </span>
            </li>
          @endforeach
        @else
